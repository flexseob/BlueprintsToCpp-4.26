...
void UGrabber::Grab()
{
	...
	if(TraceForPhysicsBodies(HitActor,HitComponent) == true)
	{
	UE_LOG(LogTemp, Warning, TEXT("Called Grab Active"));
		HitComponent->SetSimulatePhysics(true);
		GetPhysicsComponent()->GrabComponentAtLocationWithRotation(
			HitComponent, NAME_None, HitComponent->GetCenterOfMass(), FRotator());
